...
document.addEventListener("DOMContentLoaded", () => {
  ...
  avatarInputs.forEach((input, index) => {
    input.addEventListener("change", () => {
      ...
      if (!checkedRadio) {
        // If no radio button is checked, disable the button
        updateAvatarButton.disabled = true;
      } else {
        // If a radio button is checked, enable the button
  ...
  if (
    !document.querySelector(
      ".disabled .avatar-form .image-container .input-hidden:checked"
    )
  ) {
    updateAvatarButton.disabled = true;
  }
